...
function CameraCapture({ onCapture, label }) {
  ...
  if (error) {
    return <div style={{background:'var(--amber-light)',borderRadius:8,padding:'10px 14px',marginBottom:12,fontSize:12,color:'var(--amber)'}}>⚠ {error} You can still continue without a photo.
      <button className="btn-secondary btn-sm" style={{marginTop:8,width:'100%'}} onClick={()=>onCapture(null)}>Continue without photo</button>
    </div>;
  }
  ...
  return (
    <div style={{textAlign:'center',marginBottom:12}}>
      ...
      <video ref={videoRef} muted playsInline style={{width:'100%',maxWidth:280,borderRadius:8,background:'#000',transform:'scaleX(-1)'}} />
      <canvas ref={canvasRef} style={{display:'none'}} />
      <button className="btn-primary" style={{marginTop:10,width:'100%'}} onClick={capture} disabled={!ready}>
        {ready ? '📷 Take photo & continue' : 'Starting camera…'}
      </button>
    </div>
  );
}
...
function ClockModal({ onClose }) {
  ...
  async function findEmp() {
    ...
    setLoading(false); setStep('camera'); setMsg('');
  }

  async function doClock(photoData) {
    setLoading(true);
    const now = new Date();
    ...
    if (!todayPunch) {
      ...
      await db.addPunch({ employee_id:emp.id, punch_date:today, clock_in:now.toISOString(),
        effective_in:effectiveIn, scheduled_start:sched?.start_time||null, scheduled_end:sched?.end_time||null, photo_data:photoData });
      ...
      await db.clockOut(todayPunch.id, now.toISOString(), effectiveOut, photoData);
  ...
  const nowMins = now.getHours()*60+now.getMinutes();
  const actionLabel = todayPunch && !todayPunch.clock_out ? 'Clock Out' : 'Clock In';
    ...
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        {done ? (
          ...
          <div>
            <div style={{textAlign:'center',marginBottom:12}}>
              <p style={{fontSize:13,color:'var(--text-muted)'}}>{emp.name}</p>
              {sched && <p style={{fontSize:12,color:'var(--teal)'}}>Scheduled today: {sched.start_time} – {sched.end_time}</p>}
            </div>
            ...
            {loading ? (
              <div style={{textAlign:'center',padding:'2rem 0',color:'var(--text-muted)',fontSize:14}}>Saving…</div>
            ) : (
              <CameraCapture label={`Look at the camera to ${actionLabel.toLowerCase()}`} onCapture={doClock} />
            )}
          </div>
        ) : null}
      </div>
  ...
  function TimesheetsTab() {
    ...
    return (
      <div>
        ...
        <div className="card">
          <table>
            <thead><tr><th>Employee</th><th>Date</th><th>Clock In</th><th>Eff. In</th><th>Clock Out</th><th>Eff. Out</th><th>Hours</th><th>Photos</th><th></th></tr></thead>
              ...
              <tr key={p.id} style={p.adjusted?{background:'rgba(250,238,218,0.25)'}:{}}>
                ...
                <td>
                  <div style={{display:'flex',gap:4}}>
                    {p.photo_data && <img src={p.photo_data} alt="in" title="Clock-in photo" style={{width:32,height:24,objectFit:'cover',borderRadius:4,cursor:'pointer',border:'1px solid var(--border)'}} onClick={()=>setViewPhoto(p.photo_data)} />}
                    {p.photo_data_out && <img src={p.photo_data_out} alt="out" title="Clock-out photo" style={{width:32,height:24,objectFit:'cover',borderRadius:4,cursor:'pointer',border:'1px solid var(--border)'}} onClick={()=>setViewPhoto(p.photo_data_out)} />}
                  </div>
                </td>
    ...
    <div style={{maxWidth:900,margin:'0 auto',padding:'0 1rem 3rem'}}>
      ...
      {viewPhoto&&(
        <div className="modal-bg" onClick={()=>setViewPhoto(null)}>
          <div className="modal" style={{padding:8,width:'auto'}}>
            <img src={viewPhoto} alt="Clock punch" style={{display:'block',maxWidth:'80vw',maxHeight:'80vh',borderRadius:8}} />
          </div>
        </div>
      )}
    </div>
