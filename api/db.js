...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'addPunch':
        ({ rows } = await client.query(
          'INSERT INTO punches (employee_id, punch_date, clock_in, effective_in, scheduled_start, scheduled_end) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
          [params.employee_id, params.punch_date, params.clock_in, params.effective_in, params.scheduled_start, params.scheduled_end]
        ));
        break;
      case 'clockOut':
        ({ rows } = await client.query(
          'UPDATE punches SET clock_out=$1, effective_out=$2 WHERE id=$3 RETURNING *',
          [params.clock_out, params.effective_out, params.id]
        ));
        break;
      case 'updatePunch':
        ({ rows } = await client.query(
